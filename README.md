    ...
    *   Verifica si la Supabase CLI está instalada.
        ...
        *   Si aceptas, el script intentará los siguientes métodos en secuencia:
            ...
            2.  **Scoop (Solo Windows):** Si Winget falla o no está disponible, el script verificará si Scoop está instalado.
                *   Si Scoop no está instalado, el script **intentará instalar Scoop automáticamente**. Esto implica cambiar la política de ejecución de PowerShell del usuario actual y ejecutar el script de instalación oficial de Scoop desde internet. Si este paso falla debido a permisos, se te informará que necesitas **ejecutar `supabase_setup.py` como Administrador** para que la instalación de Scoop pueda proceder.
                *   Si Scoop está (o se instala exitosamente), intentará `scoop install supabase`.
            3.  **NPM (Multiplataforma):** Si los métodos anteriores fallan o no aplican, y si NPM está disponible, intentará instalar `supabase` globalmente (`npm install supabase --global`). El script no instalará Node.js/NPM; este debe ser un prerrequisito gestionado por el usuario.
            4.  **Descarga Directa desde GitHub (Multiplataforma):** Como último recurso, si todos los métodos anteriores fallan, el script intentará descargar la última versión de Supabase CLI directamente desde sus releases en GitHub. La extraerá y la colocará en un directorio dentro de tu perfil de usuario (`%LOCALAPPDATA%\\Supabase\\CLI` en Windows o `~/.supabase/bin` en Linux/macOS). En este caso, **deberás añadir manualmente este directorio a tu variable de entorno PATH** y reiniciar tu terminal para que el comando `supabase` sea accesible globalmente.
